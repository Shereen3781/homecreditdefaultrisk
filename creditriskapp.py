...
with row4_2:
    ...
    st.markdown(' ')
    st.markdown(' ')
    st.markdown(':blue[Some numerical features that seems to have relationship to the likelihood of an applicant to repay a loan are:]')
...
st.markdown(":blue[Revolving loans are just a small fraction from the total number of loans. But compared with their frequency, a larger amount of Revolving loans, are not repaid.]")
st.markdown(":blue[The number of female clients is almost double the number of male clients. But, males have a higher chance of not repaying their loans (almost 10%), comparing with women (almost 7%).]")
st.markdown(":blue[Client owns a car are almost a half of the ones that doesn't own one. But they have a lower chance of not repaying their loans than the ones that doesn't own a car.]")
st.markdown(":blue[Very few people are registered in not live. Generally, not repayment rate is slightly larger for these cases than in the rest.]")
st.markdown(":blue[Majority of the clients have Secondary / secondary special education, followed by clients with Higher education. Only a very small number having an academic degree.The Lower secondary category, although rare, have the largest rate of not returning the loan (11%).\
...
st.markdown(":blue[Oraganizations with highest percent of loans not repaid are Transport: type 3 (16%), Industry: type 13 (13.5%), Industry: type 8 (12.5%) and Restaurant (less than 12%).]")
st.markdown(":blue[Most of clients have income from Working, followed by Commercial associate, Pensioner and State servant.\
...
sns.barplot(x=ms.index, y=ms["Percent_na"],color="blue",alpha=0.8)
...
st.pyplot(fig)
st.markdown(":blue[This is correlation matrix after deleting features with jigh percentage of missing values.\
There are still some features are highly correlated so i deleted one of each highly correlated pairs.]")
